    ...
    public static function get(string $type='text', string $query='', $from=null, int $until=1): array
    {
        ...
        try{
            if (!empty($query) && $type == 'author') {
                $a = $db->prepare("SELECT id,executor,target_ip,mask,target_member,target_aclgroup,target_id,comment,`datetime`,until,`action`,granted FROM `BlockHistory` WHERE `executor`=? AND $sqlstr ORDER BY id ASC LIMIT 100");
                $a->execute([trim($query)]);
            } elseif (!empty($query) && $type == 'text') {
                $a = $db->prepare("SELECT id,executor,target_ip,mask,target_member,target_aclgroup,target_id,comment,`datetime`,until,`action`,granted FROM `BlockHistory` WHERE (`target_ip` LIKE ? OR `comment` LIKE ? OR `target_member` LIKE ? OR `target_aclgroup` LIKE ? OR `id` LIKE ? ) AND $sqlstr ORDER BY id ASC LIMIT 100");
                $q = '%'.$query.'%';
                $a->execute([$q,$q,$q,$q,$q]);
            } else {
                $a = $db->query("SELECT id,executor,target_ip,mask,target_member,target_aclgroup,target_id,comment,`datetime`,until,`action`,granted FROM `BlockHistory` WHERE $sqlstr ORDER BY id ASC LIMIT 100");
            }
