...
class NamuMark {
	...
	function __construct($wtext) {
		...
		$this->list_tag = [
			['*', 'ul pressdo-ul'],
			['1.', 'ol pressdo-ol pressdo-ol-numeric'],
			['A.', 'ol pressdo-ol pressdo-ol-capitalised'],
			['a.', 'ol pressdo-ol pressdo-ol-alphabetical'],
			['I.', 'ol pressdo-ol pressdo-ol-caproman'],
			['i.', 'ol pressdo-ol pressdo-ol-lowroman']
		];
	...
	private function lineParser($line) {
		...
		if(self::startsWith($line, '=') && preg_match('/^(=+)(.*?)(=+) *$/', trim($line), $match) && $match[1]===$match[3]) {
			...
			$result .= '<span pressdo-edit-section onclick="event.cancelBubble=true"><a href="/edit/'.$this->title.'?section='.$id.'" rel="nofollow">[편집]</a></span>
							</span>
	...
	private function formatParser($line) {
		...
		for($j=0;$j<$line_len;self::nextChar($line,$j)) {
			...
			/*if(self::startsWith($line, 'http', $j) && preg_match('/(https?:\/\/[^ ]+\.(jpg|jpeg|png|gif))(?:\?([^ ]+))?/i', $line, $match, 0, $j)) {
				...
				$j+=strlen($innerstr)-1;
				continue;
			}else*/if(self::startsWith($line, 'attachment', $j) && preg_match('/attachment:([^\/]*\/)?([^ ]+\.(?:jpg|jpeg|png|gif))(?:\?([^ ]+))?/i', $line, $match, 0, $j)) {
				// 파일
				if($this->imageAsLink)
