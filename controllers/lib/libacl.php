...
class WikiACL
{
    ...
    */
    ...
    public function check(string $access)
    {
        ...
        if($this->status === null){
            ...
            if(count($this->allow_list) < 1)
                $msg = Lang::get('msg')['aclerr_no_rules'];
            ...
            $error->message = str_replace(
                ['@1@', '@3@', '@4@', '@5@', '@6@', '@7@'], 
                [$this->access, implode(' OR ', $this->allow_list), $error->target->id, $error->target->until, $error->target->reason, self::format_cond($error->cond)], 
    ...
    private function handle_action(bool $in_cond, string $cond, string $value, string $action)
    {
        switch($action) {
            case 'allow':
                array_push($this->allow_list, $cond);
                break;
            case 'deny':
