...
class Models extends baseModels
{
    ...
    public static function RecentChanges(string|null $option)
    {
        $db = self::db();
        try {
            $d = $db->query("SELECT `docid`,`action`,`comment`,`reverted_version`,`count`,`contributor`, `acl_changed`, `moved_from`, `moved_to`, `datetime`, rev FROM `document` WHERE BINARY `is_hidden`='false' $option ORDER BY `datetime` DESC LIMIT 100");
